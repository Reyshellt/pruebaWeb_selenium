...
// Crear la subcarpeta 'incompleteCreationTest' si no existe
const creationDir = path.join(baseDir, 'incompleteCreationTest');
if (!fs.existsSync(creationDir)) {
    fs.mkdirSync(creationDir);
}

// Limpiar la carpeta 'incompleteCreationTest' eliminando todos los archivos antiguos
...
async function incompleteCreationTest() {
    ...
    try {
        ...
        // 7. Hacer clic en el botón "Facturar"
        await driver.findElement(By.css('button.btn-primary[type="submit"]')).click();
        await captureScreenshot(driver, '7_facturar_clicked.png');
...
// Función para capturar capturas de pantalla y guardarlas en la carpeta 'incompleteCreationTest'
